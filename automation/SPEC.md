# Daily sound scout — automation spec
...
| Component   | Where                                                       |
+-------------+-------------------------------------------------------------+
| Workflow    | .github/workflows/daily-sound-scout.yml                       |
...
- [ ] Manual dry run via **Actions → Daily sound scout → Run workflow**
...
1. GitHub → **Actions** → **Daily sound scout** → **Run workflow**
...
5. Keep candidates with confidence **≥ 55**, category score
   **≥ MIN_CATEGORY_SCORE (7)**, description length
   **≥ MIN_DESCRIPTION_LEN**, and no **NEGATIVE_KEYWORDS** hit
6. Rank by ToC score then confidence; apply **MAX_PER_SECTION** cap
7. **If count = 0 →** exit 0 (no PR)
8. **If count ≥ 1 →** close open `bot/daily-candidates-*` PRs, create bot
...
| 70-89       | Good fit — PR body "Keep" band (≥ KEEP_THRESHOLD)           |
| 55-69       | Borderline — PR body "Review" band; scrutinize              |
| < 55        | Skip                                                        |
+-------------+-------------------------------------------------------------+
```

**Score inputs (ToC-weighted):** category fit (primary), GitHub topic bonus,
description quality, recency, stars (secondary), notable org. Search queries
do **not** use `pushed:` filters; recency is enforced in `score_repo()`.

**High-quality bar (≥ 55):** in-scope, deduped, not archived/fork, no
negative-keyword hit, description ≥ 15 chars, ToC fit ≥ 7, passes
confidence threshold. PR selection ranks by ToC score then confidence and
caps at `MAX_PER_SECTION` (3) per Domain > Subsection.
...
**PR body:** Keep (≥70) and Review (55–69) tables with confidence, ToC
score, category, **original GitHub description**, and rationale per entry;
plus entry details and skipped list.
...
| Desc floor  | description length >= 15 (MIN_DESCRIPTION_LEN)                 |
| Negatives   | NEGATIVE_KEYWORDS hard-skip before scoring                    |
| Topics      | TOPIC_BONUS adds confidence when GitHub topics match          |
| Section cap | <= 3 entries per Domain > Subsection (MAX_PER_SECTION)        |
| Keep band   | confidence >= 70 highlighted separately in PR body            |
| Max per PR  | 10 entries (top by ToC score then confidence)               |
...
> `NEGATIVE_KEYWORDS`, `TOPIC_BONUS`, `MAX_PER_SECTION`, and
> `automation/queries.json` together.
